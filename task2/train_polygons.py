# """
# train_polygons.py (Anti-Overfitting with Class Weighting Version)

# This script trains a classifier on top of CLIP's text encoder to predict colors
# for polygons based on their textual descriptions.
# Enhancements:
# - Fixed 10-epoch training with validation monitoring.
# - Strong regularization (dropout, weight decay).
# - Balanced dataset sampling with data augmentation.
# - Using class weights (computed from basic samples) to compensate for class imbalance.
# - Weighted text merging for parent-child relationships.
# """
...
# # Expanded candidate palette with semantic mapping
# CANDIDATE_COLORS = [
#     ("black",  (0, 0, 0)),
#     ("white",  (255, 255, 255)),
#     ("red",    (255, 0, 0)),
...
# # Expanded color word sets for detection
# CANDIDATE_COLOR_NAMES = {name.lower() for (name, _) in CANDIDATE_COLORS}

# # Color synonyms for improved detection
# COLOR_SYNONYMS = {
...
# # Extended color words (not used directly but available)
...
#     # Match color descriptions (e.g., "red colored", "fill red")
...
#     """
#     Detects if a text contains a candidate color word.
...
#                             logging.debug("Found color synonym '%s' near context word '%s' -> %s", synonym, words[context_idx], color)
#                             return color
#                 visual_objects = {
#                     "sky": "blue",
#                     "grass": "green",
...
#     """
#     Returns a merged text from the child and its parent where the child's text
#     is given extra weight if it contains explicit or indicative color words.
#     """
...
#             logging.debug("Child has EXPLICIT color - HEAVILY weighted merge for shape %s", shape.get("id"))
#         elif color_word:
#             merged = (desc_child + " ") * 3 + desc_parent
#             logging.debug("Child has color word - heavily weighted merge for shape %s", shape.get("id"))
#         else:
#             merged = (desc_child + " ") * 2 + desc_parent
#             logging.debug("Balanced merge for shape %s", shape.get("id"))
#         logging.debug("Final merged text: '%s'", merged)
#         return merged
#     else:
#         logging.debug("Using only child description for shape %s", shape.get("id"))
#         return desc_child
...
#         "water": "blue"
#     }
#     for object_word, color_name in visual_mappings.items():
#         if object_word in words:
#             for i, (cand_name, _) in enumerate(CANDIDATE_COLORS):
#                 if cand_name.lower() == color_name:
#                     logging.debug("Inferred color from visual object '%s' -> %s, index=%d", object_word, color_name, i)
#                     return i

#     logging.debug("No target candidate color determined from text.")
#     return None

# def augment_dataset(sample, augment_factor=0.5):
#     """
...
#             if random.random() > (1.0 - augment_factor):
#                 augmented.append((variant, target))
#         if color_name in COLOR_SYNONYMS:
#             for synonym in COLOR_SYNONYMS[color_name]:
#                 variant = f"A {synonym} colored shape."
#                 if random.random() > (1.0 - augment_factor/2):
#                     augmented.append((variant, target))
#     return augmented
...
#     """
#     logging.debug("Computing bounding box for all shapes...")
...
# def precompute_candidate_embeddings(clip_model, preprocess, device):
#     """
#     For each candidate color, create a 224x224 solid patch and compute its CLIP image embedding.
#     Returns a list of tuples: (name, (R, G, B), embedding).
#     """
#     logging.debug("Precomputing candidate embeddings...")
#     data_list = []
#     for name, rgb in CANDIDATE_COLORS:
#         logging.debug("Processing candidate color: %s with RGB: %s", name, str(rgb))
#         patch_img = Image.new("RGB", (224, 224), rgb)
#         patch_tensor = preprocess(patch_img).unsqueeze(0).to(device)
#         with torch.no_grad():
#             feats = clip_model.encode_image(patch_tensor)
#             feats /= feats.norm(dim=-1, keepdim=True)
#         data_list.append((name, rgb, feats))
#         logging.debug("Embedding computed for candidate: %s", name)
#     return data_list

# class ShapesDataset(data.Dataset):
#     """
#     Enhanced dataset for training color prediction.
#     """
...
#         # Save basic counts for class weighting later
...
#                 text, target = sample
#                 # Compute per-sample augmentation factor: more for underrepresented classes
#                 # (Here we could also use the previously computed ratio if desired)
#                 factor = 0.5  # you can further adjust this factor if desired
#                 augmented = augment_dataset(sample, factor)
...
# def train_model(dataset, clip_model, classifier, optimizer, device, class_weights, epochs=10):
#     """
#     Train the classifier with anti-overfitting measures, including class weighting.
#     """
...
#     clip_model.eval()  # freeze CLIP evaluation mode
#     # Use weighted loss
...
# def main():
#     logging.info("Starting the anti-overfitting training script for polygon color prediction...")
#     logging.info("Using fixed 10 epochs with validation monitoring")
...
#     logging.info("Canvas size (for info): width=%d, height=%d", width, height)
    
#     device = "cuda" if torch.cuda.is_available() else "cpu"
#     logging.info("Loading CLIP model on device: %s", device)
#     clip_model, preprocess = clip.load("ViT-B/32", device=device)
#     clip_model.eval()
#     logging.info("CLIP model loaded successfully.")
#     candidate_data = precompute_candidate_embeddings(clip_model, preprocess, device)
#     logging.info("Precomputed candidate embeddings for candidate colors.")
...
#     # Compute class weights from basic samples (non-augmented distribution)
#     basic_counts = dataset.basic_color_counts
#     total_basic = dataset.total_basic_samples
#     class_weight_list = []
#     for i in range(NUM_CANDIDATES):
#         if basic_counts[i] > 0:
#             weight = total_basic / (NUM_CANDIDATES * basic_counts[i])
#         else:
#             weight = 0.0
...
#         nn.ReLU(),
#         nn.Dropout(0.3),  # regularization dropout
#         nn.Linear(128, NUM_CANDIDATES)
#     ).to(device)
    
#     optimizer = optim.Adam(classifier.parameters(), lr=5e-4, weight_decay=5e-5)
    
#     logging.info("Starting training for 10 epochs with validation split...")
#     classifier = train_model(dataset, clip_model, classifier, optimizer, device, class_weights, epochs=10)
