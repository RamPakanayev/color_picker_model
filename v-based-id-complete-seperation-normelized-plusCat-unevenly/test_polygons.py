"""
test_polygons.py (RGB Prediction Version Supporting All Model Types)

This script uses a trained model to predict RGB colors for polygons from text descriptions,
supporting original, separate pathways, and hybrid fusion architectures.
"""
...
class RGBPredictionModel(nn.Module):
    """
    Neural network for predicting RGB values with support for original, separate pathways,
    and hybrid fusion architectures.
    """
    def __init__(self, text_embed_dim, poly_feature_dim=10, hidden_dims=[256, 128], 
                output_dim=3, dropout_rates=[0.4, 0.3], model_type="original"):
        ...
        if model_type == "original":
            # Original model with early fusion (concatenation)
            ...
            logging.debug(f"Original model input dimension: {prev_dim} = 2*{text_embed_dim} (text) + {self.poly_feature_processed_dim} (polygon)")
            ...
            logging.info(f"Created original model with concatenated inputs")
            
        elif model_type == "separate_pathways":
            # Model with completely separate pathways for each input
            ...
            # Initialize with equal contribution from each pathway
            self.combination_weights = nn.Parameter(torch.ones(3) / 3.0)
            
            logging.info(f"Created model with completely separate pathways for parent, polygon, and child")
            
        elif model_type == "hybrid_fusion":
            # Hybrid model with both separate pathways and combined pathway
    ...
    def forward(self, parent_embed, child_polygon, child_embed):
        """
        Forward pass with model-type dependent processing
        ...
        """
        # Process polygon features first (common to all model types)
        poly_processed = self.poly_net(child_polygon)
        
        if self.model_type == "original":
            # Original model implementation (early fusion)
        ...
        elif self.model_type == "separate_pathways":
            # Separate pathways implementation (late fusion)
        ...
        elif self.model_type == "hybrid_fusion":
            # Hybrid fusion implementation (multi-level fusion)
            parent_rgb = self.parent_pathway(parent_embed)
            polygon_rgb = self.polygon_pathway(poly_processed)
            child_rgb = self.child_pathway(child_embed)
            ...
            raise ValueError(f"Unknown model type in forward pass: {self.model_type}")
...
def get_color_prediction(parent_description, child_polygon, child_description, clip_model, rgb_model, device):
    ...
    with torch.no_grad():
        ...
        # Run through RGB model to get RGB prediction
...
def load_model(model_path, model_info_path, device):
    ...
    try:
        if os.path.exists(model_info_path):
            ...
            logging.info(f"Loaded model architecture from info file. Model type: {model_type}")
        else:
    ...
    try:
        ...
        # Log the weights if this model has them
        if model_type == "separate_pathways":
            with torch.no_grad():
                weights = torch.softmax(model.combination_weights, dim=0)
                logging.info(f"Separate pathways weights: Parent={weights[0].item():.4f}, "
                           f"Polygon={weights[1].item():.4f}, Child={weights[2].item():.4f}")
        elif model_type == "hybrid_fusion":
            with torch.no_grad():
                weights = torch.softmax(model.combination_weights, dim=0)
                logging.info(f"Hybrid fusion weights: Parent={weights[0].item():.4f}, "
                           f"Polygon={weights[1].item():.4f}, Child={weights[2].item():.4f}, "
                           f"Combined={weights[3].item():.4f}")
    except Exception as e:
...
def main():
    ...
    parser.add_argument("--model", default="rgb_model_hybrid_weights.pth", 
                       help="Path to trained model weights")
    parser.add_argument("--model-info", default="rgb_model_hybrid_info.json",
