...
class RGBPredictionModel(nn.Module):
    ...
    and polygon features with separate processing paths.
    """
    def __init__(self, text_embed_dim, poly_feature_dim=10, hidden_dims=[256, 128], 
        ...
        # Calculate dimensions to ensure they sum to hidden_dims[0] exactly
        parent_dim = hidden_dims[0] // 3
        child_dim = hidden_dims[0] // 3
        # Make poly_dim slightly larger to ensure total is exactly hidden_dims[0]
        poly_dim = hidden_dims[0] - parent_dim - child_dim
        ...
        # Process parent embedding separately
        self.parent_net = nn.Sequential(
            nn.Linear(text_embed_dim, parent_dim),
            nn.ReLU(),
            nn.Dropout(dropout_rates[0])
        )
        
        # Process child embedding separately
        self.child_net = nn.Sequential(
            nn.Linear(text_embed_dim, child_dim),
            nn.ReLU(),
            nn.Dropout(dropout_rates[0])
        )
        
        # Process polygon features further
        self.poly_processed_net = nn.Sequential(
            nn.Linear(self.poly_feature_processed_dim, poly_dim),
            nn.ReLU(),
            nn.Dropout(dropout_rates[0])
        )
        
        # Create network that combines processed inputs
        self.combined_net = nn.Sequential(
            nn.Linear(hidden_dims[0], hidden_dims[1]),
            nn.ReLU(),
            nn.Dropout(dropout_rates[1]),
            nn.Linear(hidden_dims[1], output_dim),
            nn.Sigmoid()  # Output between 0 and 1, will scale to 0-255 later
        )
        
        logging.info(f"Model input dimensions: parent={text_embed_dim}, polygon={self.poly_feature_processed_dim}, child={text_embed_dim}")
        logging.info(f"Network path output dimensions: parent={parent_dim}, polygon={poly_dim}, child={child_dim}, total={parent_dim+poly_dim+child_dim}")
    
    def forward(self, parent_embed, child_polygon, child_embed):
        """
        Forward pass with three separate inputs: parent description, child polygon, and child description
        ...
        """
        # Process each input through its own network path
        poly_processed = self.poly_net(child_polygon)
        parent_processed = self.parent_net(parent_embed)
        child_processed = self.child_net(child_embed)
        poly_final = self.poly_processed_net(poly_processed)
        
        # Combine processed outputs
        combined = torch.cat([parent_processed, poly_final, child_processed], dim=1)
        
        # Forward through combined network
        raw_output = self.combined_net(combined)
...
def main():
    ...
    logging.info(f"Created RGB prediction model with three separate paths for: parent description, child polygon, and child description")
