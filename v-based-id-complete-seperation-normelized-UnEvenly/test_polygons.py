"""
test_polygons.py (RGB Prediction Version for Completely Separate Pathways)

This script uses a trained model with completely separate pathways to predict RGB colors
for polygons from text descriptions, handling parent-child relationships.
"""
...
log_file = os.path.join(logs_dir, f"test_rgb_polygons_separate_{timestamp}.log")
...
class RGBPredictionModel(nn.Module):
    ...
    def __init__(self, text_embed_dim, poly_feature_dim=10, hidden_dims=[256, 128], 
                output_dim=3, dropout_rates=[0.4, 0.3], model_type="separate_pathways"):
        ...
        if model_type == "separate_pathways":
            ...
            # Initialize combination weights with strong bias toward the child description pathway.
            # Softmax([1.0, 1.0, 4.0]) ≈ [0.063, 0.063, 0.873] gives ~87% weight to child_rgb
    ...
    def forward(self, parent_embed, child_polygon, child_embed):
        ...
        """
        if self.model_type == "separate_pathways":
...
def load_model(model_path, model_info_path, device):
    ...
    try:
        ...
        # If this is a separate pathways model, log the learned weights
        if model_type == "separate_pathways":
            with torch.no_grad():
                weights = torch.softmax(model.combination_weights, dim=0)
                logging.info(f"Pathway contribution weights: Parent={weights[0].item():.4f}, "
...
def main():
    ...
    parser.add_argument("--model", default="rgb_model_separate_weights.pth", 
                       help="Path to trained model weights")
    parser.add_argument("--model-info", default="rgb_model_separate_info.json",
